...
using Assets.Sceelix.Processors;
...
public class SceelixConnectorWindow : EditorWindow
{
    ...
    private static Dictionary<String, MessageProcessorAttribute> _messageProcessorAttributes;
    ...
    public void Update()
    {
        ...
        if (_messageProcessorAttributes == null)
        {
    ...
    public void OnGUI()
    {
        ...
        _storePhysicalAssets = EditorGUILayout.Toggle(new GUIContent("Store Physical Assets:","Indicates if, when transfering textures, materials and meshes, if these should be persisted to disk (will be stored under 'SceelixAssets'). Doing this takes more time but may be useful in some cases. If disabled, these assets will only be stored in memory and directly in the project."), _storePhysicalAssets);
        ...
        GUILayout.Label(new GUIContent("Clear Unused","Removed all generated physical assets that are not being referenced in any scene."),labelStyle);
    ...
    public void Start(String hostName, int port, int poolPeriod)
    {
        ...
        try
        {
            _messageClient = new MessageClient(hostName, port, poolPeriod);
            _messageClient.RawMessageReceived += OnRawMessageReceived;
            _messageClient.ParsedMessageReceived += MessageClientOnParsedMessageReceived;
    ...
    private void MessageClientOnParsedMessageReceived(JToken token)
    {
        MessageProcessorAttribute messageProcessorAttribute;
        ...
        if (_messageProcessorAttributes.TryGetValue(subject, out messageProcessorAttribute))
        {
            ...
            messageProcessorAttribute.Invoke(editorGenerationContext, data);
        }
        else
        {
    ...
    {
        _messageProcessorAttributes = FunctionProcessorAttribute.GetFunctionsWithAttribute<MessageProcessorAttribute>();
